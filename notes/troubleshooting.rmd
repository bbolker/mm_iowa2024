...
gg1 <- tt |> dplyr::filter(effect == "fixed") |>
    ggplot(aes(y = term, x = estimate, colour = optimizer)) +
...
tt2 <- (tt
    ...
    |> dplyr::mutate(is_cor = startsWith(term, "cor"))
)
gg2 <- 
    ggplot(tt2, aes(y = term, x = estimate, colour = optimizer)) +
    geom_point(position = pd) +
    facet_wrap(~is_cor, scale = "free", labeller = label_both)
