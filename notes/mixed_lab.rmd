...
One good way to assess the results of a model fit is to look at a coefficient plot: `dotwhisker::dwplot()` does this. *Note that it automatically scales all coefficients by 2SD; use `by_2sd=FALSE` to leave coefficients unscaled*
...
Can use the algorithm that `lme` uses:
```{r calcDenDF}
